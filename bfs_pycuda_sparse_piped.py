...
def sparse_matrix_multiply_pycuda(A, B, num_warmup=2, num_test_runs=5, stream=None):
    ...
    # Copy data to GPU
    if stream:
        cuda.memcpy_htod_async(A_data_gpu, A_data, stream)
        cuda.memcpy_htod_async(A_indices_gpu, A_indices, stream)
        cuda.memcpy_htod_async(A_indptr_gpu, A_indptr, stream)
        cuda.memcpy_htod_async(B_data_gpu, B_data, stream)
        cuda.memcpy_htod_async(B_indices_gpu, B_indices, stream)
        cuda.memcpy_htod_async(B_indptr_gpu, B_indptr, stream)
    else:
    ...
    try:
        ...
        if stream:
            cuda.memcpy_dtoh_async(C_dense, C_gpu, stream)
            stream.synchronize()
        else:
...
def process_cluster_pipelined(cluster_queue, result_dict, lock, stream_manager):
    """Process clusters with true GPU parallelization using CUDA streams"""
    with CUDAContextManager():
        while True:
            try:
                ...
                result, cluster_time, _ = sparse_matrix_multiply_pycuda(
                    sub_adj, sub_feat, num_warmup=1, num_test_runs=1, stream=stream)
                
                end_event.record(stream)
                stream.synchronize()  # Only synchronize this stream
                
                with lock:
                    result_dict[idx] = (result[:len(nodes_idx)], 
                                      nodes_idx, 
...
for graph_info in graphs:
    ...
    try:
        ...
        with CUDAContextManager():
            ...
            # Set up pipeline processing with CUDA streams
            cluster_queue = queue.Queue()
            result_dict = {}
            lock = threading.Lock()
            stream_manager = CUDAStreamManager(num_streams=4)  # Create stream pool
            
            # Fill queue with cluster data
            for idx, data in enumerate(cluster_data):
                cluster_queue.put((idx, data))
            
            # Create worker threads for pipelined processing
            workers = []
            num_workers = min(4, len(cluster_data))  # Adjust based on GPU capability
            for _ in range(num_workers):
                worker = threading.Thread(
                    target=process_cluster_pipelined,
                    args=(cluster_queue, result_dict, lock, stream_manager)
                )
                worker.start()
            ...
            result = result / node_counts[:, np.newaxis]
        ...
        results.append({
            ...
            "method": "pycuda_sparse_bfs_piped",
