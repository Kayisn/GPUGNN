...
class OccupancyTracker:
    ...
    def calculate_occupancy(self, kernel_function, block_size):
        ...
        max_warps_per_sm = self.device.get_attribute(
            cuda.device_attribute.MAX_WARPS_PER_MULTIPROCESSOR
        )
        ...
        if shared_mem_per_block > 0:
            blocks_by_shared_mem = (
                self.attributes['max_shared_memory_per_block'] 
    ...
    def suggest_block_size(self, kernel_function, min_blocks=1):
        ...
        """
        max_threads = min(
            self.attributes['max_threads_per_block'],
            kernel_function.max_threads_per_block
        )
        ...
        for block_x in range(self.attributes['warp_size'], max_threads + 1, self.attributes['warp_size']):
            for block_y in [1, 2, 4, 8, 16, 32]:
                ...
                if (stats.occupancy_percentage > best_occupancy and 
                    stats.active_blocks_per_sm >= min_blocks):
