...
def sparse_matrix_multiply_pycuda(A, B, index, num_warmup):
    ...
    try:
        ...
        sparse_matmul = load_gpu_func(kernel_name)
        ...
        cuda.memcpy_htod_async(A_data_gpu, A_data, stream1)
        cuda.memcpy_htod_async(A_indices_gpu, A_indices, stream1)
        cuda.memcpy_htod_async(A_indptr_gpu, A_indptr, stream1)
        cuda.memcpy_htod_async(B_data_gpu, B_data, stream2)
        cuda.memcpy_htod_async(B_indices_gpu, B_indices, stream2)
        cuda.memcpy_htod_async(B_indptr_gpu, B_indptr, stream2)
...
def calculate_pagerank_gpu(adjacency_matrix, damping=0.85, max_iterations=100, tolerance=1e-6):
    ...
    pagerank_kernel = load_gpu_func("pagerank_iteration")
