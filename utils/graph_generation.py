...
def add_real_world_graphs(num_samples=3, sample_size=200):
    ...
    for i in range(num_samples):
        sampled_nodes = np.random.choice(
            list(G_karate.nodes()), size=min(sample_size, G_karate.number_of_nodes()), replace=False
        )
        G_sample = G_karate.subgraph(sampled_nodes).copy()
        graphs.append(
            {
                "graph": G_sample,
                "name": f"Karate-Club-Sample-{i+1}",
                "type": "real-world-sample",
    ...
    graphs.append(
        {
            "graph": G_les_miserables,
            "name": "Les-Miserables",
            "type": "real-world",
            "num_nodes": G_les_miserables.number_of_nodes(),
            "sparsity": None,
        }
    )
    ...
    for i in range(num_samples):
        sampled_nodes = np.random.choice(
            list(G_les_miserables.nodes()), size=min(sample_size, G_les_miserables.number_of_nodes()), replace=False
        )
        G_sample = G_les_miserables.subgraph(sampled_nodes).copy()
        graphs.append(
            {
                "graph": G_sample,
                "name": f"Les-Miserables-Sample-{i+1}",
                "type": "real-world-sample",
                "num_nodes": G_sample.number_of_nodes(),
                "sparsity": None,
            }
...
def save_graphs(graphs):
    """
    Save the generated graphs to the graphs directory.
...
if __name__ == "__main__":
    ...
    # Print confirmation
    print(
        f"Generated and saved {len(graphs)} graphs including synthetic, real-world, and sampled subgraphs with metadata and feature matrices."
    )
